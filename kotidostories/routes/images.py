...
def get_profile_image(user_id=None, img=None):
    ...
    return send_from_directory(os.path.join(root_dir, 'kotidostories', 'pictures', 'user', user_id), img)
...
def get_post_image(user_id=None, post_id=None, img=None):
    ...
    return send_from_directory(
        os.path.join(root_dir, 'kotidostories', 'pictures', 'post', user_id, post_id),
        img)
...
def get_default_image(type=None, img=None):
    root_dir = os.path.dirname(os.getcwd())
    return send_from_directory(
        os.path.join(root_dir, 'kotidostories', 'pictures', type),
        img)
